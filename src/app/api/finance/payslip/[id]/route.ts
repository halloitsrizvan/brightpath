...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        // Fetch all salaries
        const salaries = await Salary.find({ _id: { $in: ids } }).populate('teacherId');
        if (salaries.length === 0) return NextResponse.json({ message: "Salary records not found" }, { status: 404 });

        const teacher = salaries[0].teacherId;
        ...
        for (const salary of salaries) {
            ...
            try {
                ...
                if (fs.existsSync(logoPath)) {
                    ...
                    page.drawImage(logoImage, { x: 50, y: height - 120, width: logoDims.width, height: logoDims.height });
                    page.drawText('BrightPath', { x: 50 + logoDims.width + 10, y: height - 80, size: 24, font: boldFont, color: primaryColor });
                }
            } catch (e) {}

            const safeStr = (str: any) => String(str || '').replace(/₹/g, 'INR');

            page.drawText('PAYSLIP', { x: width - 150, y: height - 90, size: 28, font: boldFont, color: primaryColor });
            y = height - 150;
            ...
            for (const cls of sortedClasses) {
                ...
                if (alternate) page.drawRectangle({ x: 50, y: y - 12, width: 500, height: 20, color: lightGray });
                ...
                page.drawText(safeStr(`${(cls.durationMinutes / 60).toFixed(2)} hrs`), { x: col4, y: y - 5, size: 9, font });
                y -= 20;
                alternate = !alternate;
            }

            const footerY = 50;
            page.drawText('This is a formal confirmation of monthly faculty remuneration.', { x: width / 2 - 140, y: footerY + 15, size: 8, font, color: darkGray });
            page.drawText('BrightPath Learning | Transparency in Education', { x: width / 2 - 100, y: footerY, size: 8, font: boldFont, color: primaryColor });
        }
