    ...
    return (
        <section className="py-32 bg-gray-50/50">
            <div className="container mx-auto px-6 text-center mb-20">
                ...
                <h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-none">The Subjects <span className="text-secondary">Spectrum.</span></h2>
            </div>

            <div className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {subjects.map((s) => (
                    <div key={s.id} className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-xl shadow-gray-200/20 hover:shadow-primary/10 transition-all group overflow-hidden relative">
                        ...
                        <div className={`w-14 h-14 ${s.color} text-white rounded-2xl flex items-center justify-center mb-6 shadow-lg rotate-3 group-hover:rotate-6 transition-all`}>
                            {s.icon}
                        </div>
                        <h3 className="text-2xl font-black text-gray-800 italic uppercase tracking-tighter mb-4 leading-none">{s.name}</h3>
